package edu.iu.kmexample.bcastreduce;
	 ...
	 private void computation(Table<DoubleArray> cenTable, Table<DoubleArray> previousCenTable,ArrayList<DoubleArray> dataPoints){
	  ...
	  private void runKmeans(List<String> fileNames, Configuration conf, Context context) throws IOException {
		  ...
		  for(int iter=0; iter < iteration; iter++){
			  ...
			  System.out.println("\n\nAfter computation and before reduce\n\n");
			  printTable(cenTable);
			  /****************************************/
			  reduce("main", "reduce_"+iter, cenTable, this.getMasterID());
			  System.out.println("\n\nAfter reduce and before calculate centroids\n\n");
			  printTable(cenTable);
			  if(this.isMaster())
				  calculateCentroids(cenTable);
				  
				System.out.println("\n\nAfter calculate centroids and before broadcast\n\n");
			  printTable(cenTable);
