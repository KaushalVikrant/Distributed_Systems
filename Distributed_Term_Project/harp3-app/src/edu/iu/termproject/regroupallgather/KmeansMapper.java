package edu.iu.termproject.regroupallgather;
...
public class KmeansMapper extends CollectiveMapper<String, String, Object, Object> implements Runnable{
	...
	KmeansMapper(int start, int end, int threadNumber, Table<MiniBatch> previousCenTable, ArrayList<MiniBatch> dataPoints , Table<MiniBatch> partialTable)
	{
	  this.start = start;
	  this.end = end;
	  this.threadNumber = threadNumber;
	  this.previousCenTable = previousCenTable;
	  this.dataPointsThread = dataPoints;
	  this.parCenTable = partialTable;
	}
	...
	public void run()
	{
		...
		for( int dpCounter = start ; dpCounter<=end; dpCounter++){
		  
		  MiniBatch dp = dataPointsThread.get(dpCounter);
		  ...
		  MiniBatch obj=dp;
		    ...
		    dp.setAllocatedCentroid(nearestPartitionID);
				...
				if(parCenTable.getPartition(nearestPartitionID) == null){
				  
				  
					Partition<MiniBatch> tmpAp = new Partition<MiniBatch>(nearestPartitionID, dp);
					parCenTable.addPartition(tmpAp);
					 
				}else{
				 
				   MiniBatch centroidMB = (MiniBatch)parCenTable.getPartition(nearestPartitionID).get();
			  ...
			  ////////////////////////////////////////////////////////////////////
			  //We can handle centroid table size here if batch is small and all partitions are not allocated
			  
			  int newCensize=parCenTable.getNumPartitions();
      		...
      		{
      		  if(parCenTable.getPartition(cou) == null)
      		  {
					    parCenTable.addPartition(previousCenTable.getPartition(cou));
      		  }
      		}
			  }
	}
	...
	protected void setup(Context context) throws IOException, InterruptedException {
		...
		System.out.println("---------------REGROUP-ALLGATHER----------------");
	...
	protected void mapCollective( KeyValReader reader, Context context) throws IOException, InterruptedException {
	 ...
	 private void computation(Table<MiniBatch> cenTable, Table<MiniBatch> previousCenTable,ArrayList<MiniBatch> dataPoints) 
	 {
	   ...
	   int processors = (int)  (Runtime.getRuntime().availableProcessors() * 0.7);
	   int dataPointsSize = dataPoints.size();
	   int perThread = dataPointsSize / processors;
	   int leftPoints = dataPointsSize % processors;
	   //System.out.println("\nNo. of Processors: "+processors+"\nNo. of dataPoints: "+dataPointsSize+"\nData Points per thread: "+perThread+"\nLeft Points: "+leftPoints);
	   int startIndex = 0;
	   int endIndex = perThread - 1;
	   
	   
	   KmeansMapper[] thread = new KmeansMapper[processors];
	   
	   Thread[] t = new Thread[processors];
	   
	   for(int loop = 0; loop < processors; loop++)
	   {
	     Table<MiniBatch> partialTable = new Table(0, new MiniBatchPlus());
	     if(loop+1 == processors)
	     {
	       endIndex += leftPoints;
	     }
	     thread[loop] = new KmeansMapper(startIndex, endIndex, loop, previousCenTable, dataPoints, partialTable);
	     t[loop] = new Thread(thread[loop], loop+"");
	     
	     //thread[loop].start();
	     startIndex += perThread;
	     endIndex += perThread;
	     t[loop].start();
	   ...
	   for(int loop = 0; loop < processors; loop++)
	   {
	    ...
	    {
	      System.out.println("Exception in start method: "+ex);
	    }
	   ...
	   for(KmeansMapper partialResult : thread)
	   {
	     Table<MiniBatch> partialTableThread = partialResult.parCenTable;
	     
	      int size=partialTableThread.getNumPartitions();
		
		for(int cou=0;cou<size;cou++)
		{
		  
			MiniBatch dp = (MiniBatch)partialTableThread.getPartition(cou).get();
		 
		  int nearestPartitionID = cou;
	  ...
	  private void runKmeans(List<String> fileNames, Configuration conf, Context context) throws IOException {
		  ...
		  int created = (int)  (Runtime.getRuntime().availableProcessors() * 0.7);
		  System.out.println("No of threads can be created: "+processors+"\n\n");
		  System.out.println("No of threads created: "+created+"\n\n");
